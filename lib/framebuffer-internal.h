...
private:
  ...
  // Private copy used by the SPWM upload path. Allocated on first SPWM dump so
  // non-SPWM framebuffers do not pay for an unused second bitplane buffer.
  // The snapshot gives the slow upload a stable source after the copy; whole
  // application-frame consistency still requires drawing offscreen and using
  // SwapOnVSync().
