...
// read RGB2. On this 64S layout, the left half leaves on RGB2 and the right half
// leaves on RGB1.
...
bool spwm_should_split_double_row_upload(
  ...
  return spwm_upload_rows == spwm_framebuffer_view.rows &&
         spwm_framebuffer_view.double_rows > 0 &&
         spwm_framebuffer_view.columns > 1 &&
         spwm_framebuffer_view.columns % 2 == 0 &&
...
void spwm_upload_framebuffer_blocks(
  ...
  // The framebuffer width includes every chained panel, while default_columns
  // remains the width of one panel. Split each panel into its own RGB1/RGB2
  // halves instead of splitting the complete chain only once.
  const bool spwm_use_panel_local_lane_mapping =
      spwm_split_double_row_upload &&
      spwm_settings.default_columns > 1 &&
      spwm_settings.default_columns % 2 == 0 &&
      spwm_framebuffer_view.columns % spwm_settings.default_columns == 0;
  const int spwm_split_panel_columns =
      spwm_use_panel_local_lane_mapping
          ? spwm_settings.default_columns
          : spwm_framebuffer_view.columns;
      ...
      for (int spwm_chip = 0; spwm_chip < spwm_chip_count; ++spwm_chip) {
        ...
        if (spwm_split_double_row_upload) {
          ...
          spwm_combine_split_double_row_upload_blocks(
              ...
              spwm_framebuffer_view.double_rows, h);
        } else {
...
void spwm_dump_to_matrix(GPIO *io, const HardwareMapping &h,
  ...
  const int spwm_effective_scan_rows =
      spwm_get_effective_scan_rows(spwm_blank_clock_row_transport,
                                   spwm_upload_rows);
  // A full-height scan override (for example --led-spwm-scan=64 on a 64-row
  // blank-clock panel) also expands the upload loop to one row per physical
  // row. Smaller overrides, such as --led-spwm-scan=43 on an 86-row panel, keep
  // the existing double-row upload count.
  const int spwm_effective_upload_rows =
      (spwm_blank_clock_row_transport &&
       spwm_effective_scan_rows == spwm_framebuffer_view.rows &&
       spwm_effective_scan_rows > spwm_upload_rows)
  ...
  // When the scan override equals the full panel height, split the normal
  // double-row framebuffer into one upload row per physical row for 64S panels.
