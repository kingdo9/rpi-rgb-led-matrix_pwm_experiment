...
// Materialize the built-in main config with per-slot latch timing. Generated
...
// Overlay one generated rotating profile during initial configuration. This
...
// Overlay every fixed slot from one generated profile as an atomic logical
...
bool spwm_apply_fixed_register_profile(
  ...
  // Validate every slot before changing any of them so a generated profile is
...
// CLI selectors and regtype names are 1-based; C++ profile arrays are 0-based.
template <typename Profile, size_t N>
const Profile *spwm_get_selected_generated_profile(
    const Profile (&spwm_profiles)[N], int spwm_register_config) {
  if (spwm_register_config <= 0 ||
      static_cast<size_t>(spwm_register_config) > N) {
    return nullptr;
  }
  return &spwm_profiles[spwm_register_config - 1];
}

// Match the same case-insensitive panel-name prefixes accepted by the SPWM
// profile registry, including suffixed hardware variants.
bool spwm_register_panel_type_matches(const char *spwm_panel_type,
                                      const char *spwm_expected_type) {
  if (spwm_panel_type == nullptr || spwm_expected_type == nullptr) return false;
  return strncasecmp(spwm_panel_type, spwm_expected_type,
                     strlen(spwm_expected_type)) == 0;
}
...
// Keep these built-in main arrays synchronized with MAIN_PROFILE in
// extract-fm6373-registers.py. Generated regtype1 must remain byte-for-byte
// identical so --led-spwm-register-config values -1, 0, and 1 are equivalent.
...
// Keep these arrays synchronized with MAIN_PROFILE in the ICND1065L extractor;
// the generated regtype1 is the diagnostic copy of this built-in main block.
...
// Keep these arrays synchronized with MAIN_PROFILE in the SM16380SH extractor;
// the generated regtype1 is the diagnostic copy of this built-in main block.
...
// Keep this built-in baseline synchronized with MAIN_PROFILE in the FM6363
// extractor so generated regtype1 remains the exact diagnostic equivalent.
static const SPWM_Register_Config_Entry
...
// Keep these five values synchronized with MAIN_PROFILE in the FM6353
// extractor so generated regtype1 remains the exact diagnostic equivalent.
static const SPWM_Register_Config_Entry
...
size_t spwm_get_panel_register_profile_count(const char *spwm_panel_type) {
  if (spwm_register_panel_type_matches(spwm_panel_type, "fm6373")) {
    return FM6373_REGISTER_PROFILE_COUNT;
  }
  if (spwm_register_panel_type_matches(spwm_panel_type, "icnd1065l")) {
    return ICND1065L_REGISTER_PROFILE_COUNT;
  }
  if (spwm_register_panel_type_matches(spwm_panel_type, "sm16380sh")) {
    return SM16380SH_REGISTER_PROFILE_COUNT;
  }
  if (spwm_register_panel_type_matches(spwm_panel_type, "fm6363")) {
    return FM6363_REGISTER_PROFILE_COUNT;
  }
...
// overlays identically numbered generated profile data; row transport does not
...
SPWM_Config spwm_create_fm6373_config(
  ...
  const SPWM_RGB_Register_Profile_View *spwm_profile =
      spwm_get_selected_generated_profile(FM6373_REGISTER_PROFILES,
...
SPWM_Config spwm_create_icnd1065l_config(
  ...
  const SPWM_RGB_Register_Profile_View *spwm_profile =
      spwm_get_selected_generated_profile(ICND1065L_REGISTER_PROFILES,
                                          spwm_register_config);
  if (spwm_profile != nullptr) {
    spwm_apply_rgb_register_profile(&spwm_config, *spwm_profile);
  }
...
SPWM_Config spwm_create_sm16380sh_config(
  ...
  const SPWM_RGB_Register_Profile_View *spwm_profile =
      spwm_get_selected_generated_profile(SM16380SH_REGISTER_PROFILES,
                                          spwm_register_config);
  if (spwm_profile != nullptr) {
    spwm_apply_rgb_register_profile(&spwm_config, *spwm_profile);
  }
...
SPWM_Config spwm_create_fm6363_config(
  ...
  const SPWM_Fixed_Register_Profile_View *spwm_profile =
      spwm_get_selected_generated_profile(FM6363_REGISTER_PROFILES,
                                          spwm_register_config);
  if (spwm_profile != nullptr) {
...
SPWM_Config spwm_create_fm6353_config(
  ...
  const SPWM_Fixed_Register_Profile_View *spwm_profile =
      spwm_get_selected_generated_profile(FM6353_REGISTER_PROFILES,
                                          spwm_register_config);
  if (spwm_profile != nullptr) {
    spwm_apply_fixed_register_profile(&spwm_config, *spwm_profile);
  }
