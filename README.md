...
- `--led-spwm-register-config=<0|1..N>` selects the register payload. `0` uses the compiled main config; a positive value selects the identically numbered runtime-catalog `regtypeN` profile used by Demo 15. The available upper limit depends on `--led-panel-type`.
...
- `--led-spwm-data-layout=<0..5>` controls full-height RGB routing for row-address types 1/2. `0` uses the panel profile; FM6373 currently resolves it to layout 1. `1` maps each panel's left/right halves to RGB2/RGB1 and `2` swaps those lanes. `3` keeps a full-width serial column stream and copies it to RGB1 and RGB2; `4` and `5` send that stream only on RGB1 or RGB2. Layouts 1..5 require scan to equal rows and cannot be combined with `--led-multiplexing`; split layouts 1/2 also require panel columns divisible by 32 and use `--led-cols` to divide each chained panel independently. If layout 1 repeats the right half of Demo 3 and layout 2 repeats the left half, try serial layout 4; layout 3 broadcasts the same stream when the active RGB bus is unknown. The ICND1065L 86-row/43-scan and 64-row/32-scan examples remain on layout 0's paired-row upload.
